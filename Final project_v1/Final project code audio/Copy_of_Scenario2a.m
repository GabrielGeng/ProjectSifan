...
Y_trans = fft(Y,256);
X_trans = fft(x(1:2,:).',256).';
trans = Y_trans./X_trans;
mt_f=linspace(1,4000,128);
b = beamformer;
J = 2;                  % Number of sensors
dx =0.0;                 % meters of element spacing in x-direction
dy = 0.01;                 % meters of element spacing in y-direction
my_array = arrays.ULA_x1x2(J,dx,dy);
set(b, 'array',         my_array);
set(b, 'angles',        -180:1:180);
set(b, 'mt_frequency',  mt_f);
b.mt_weights = trans(:,1:128);
...
soundsc(Y);
% audiowrite('RLS 2sensor.wav',Y,8000);
Y_trans = fft(Y,256);
X_trans = fft(x(1:2,:).',256).';
trans = Y_trans./X_trans;
mt_f=linspace(1,4000,128);
